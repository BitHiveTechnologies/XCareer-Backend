...
class NotificationService {
  ...
  async createBulkNotifications(notifications: NotificationData[]): Promise<any[]> {
    try {
      ...
      logger.error('Create bulk notifications failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
   ...
   */
  async createJobAlertNotification(userId: string, jobId: string, jobData?: any): Promise<any> {
    try {
      let job;
      if (jobData) {
        job = jobData;
      ...
      logger.error('Create job alert notification failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        userId,
        jobId
      });
  ...
  async createSubscriptionNotification(
    userId: string,
    type: 'activated' | 'expired' | 'cancelled' | 'payment_failed' | 'renewal_reminder',
    ...
    try {
      ...
      logger.error('Create subscription notification failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        userId,
        type
      });
  ...
  async createPaymentNotification(
    ...
    try {
      ...
      logger.error('Create payment notification failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        userId,
        type
      });
  ...
  async createApplicationNotification(
    ...
    try {
      ...
      if (applicationData) {
        ...
        application = await JobApplication.findById(applicationId).populate('jobId');
        if (!application) {
      ...
      logger.error('Create application notification failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        userId,
        applicationId,
        status
  ...
  async createSystemNotification(
    ...
    try {
      ...
      logger.error('Create system notification failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        userId,
        title
      });
  ...
  async createProfileNotification(
    userId: string,
    type: 'incomplete' | 'complete' | 'update_required'
  ): Promise<any> {
    try {
      let title: string;
      let message: string;
      ...
      logger.error('Create profile notification failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        userId,
        type
      });
  ...
  async cleanupExpiredNotifications(): Promise<number> {
    try {
      ...
      logger.error('Cleanup expired notifications failed', {
        error: error instanceof Error ? error.message : 'Unknown error'
      });
      throw error;
    }
  ...
  async getAdminStats(): Promise<any> {
    try {
      ...
      logger.error('Get admin notification stats failed', {
        error: error instanceof Error ? error.message : 'Unknown error'
      });
      throw error;
    }
