...
export class UserProvisioningService {
   ...
   * Create a new user
   */
  private static async createUser(data: UserProvisioningData): Promise<{ user: any, password?: string }> {
    try {
   ...
   */
  private static async triggerWelcomeEmail(user: any, data: UserProvisioningData, password?: string): Promise<void> {
    try {
      ...
      if (password) {
        const emailSent = await emailService.sendSubscriptionWelcomeCredentialsEmail(
          user.email,
          data.profileData?.firstName || 'User',
          password,
          data.subscriptionPlan || 'basic'
        );
        
        if (emailSent) {
          logger.info('Welcome credentials email sent successfully', { userId: user._id, email: user.email });
        } else {
          logger.warn('Welcome credentials email failed to send', { userId: user._id, email: user.email });
        }
        return;
      }
