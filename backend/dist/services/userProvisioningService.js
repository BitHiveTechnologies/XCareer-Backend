...
class UserProvisioningService {
    ...
    static async provisionUser(data) {
        ...
        try {
            ...
            const { user, password: generatedPassword } = await this.createUser(data);
            if (!user) {
            ...
            // Step 5: Trigger welcome email with credentials
            await this.triggerWelcomeEmail(user, data, generatedPassword);
            result.success = true;
    ...
    static async createUser(data) {
        try {
            ...
            // Add Clerk user ID if provided, otherwise generate a secure password
            if (data.clerkUserId) {
                ...
                generatedPassword = crypto.randomBytes(8).toString('hex'); // 16 characters
     ...
     */
    static async triggerWelcomeEmail(user, data, password) {
        try {
            ...
            // If password is provided, send the new subscription welcome credentials email
            if (password) {
                const emailSent = await emailService.sendSubscriptionWelcomeCredentialsEmail(user.email, data.profileData?.firstName || 'User', password, data.subscriptionPlan || 'basic');
                if (emailSent) {
                    logger_1.logger.info('Welcome credentials email sent successfully', { userId: user._id, email: user.email });
                }
                else {
                    logger_1.logger.warn('Welcome credentials email failed to send', { userId: user._id, email: user.email });
                }
                return;
            }
