...
exports.getMatchingStatistics = exports.getJobRecommendations = exports.findMatchingJobsForUser = exports.findMatchingUsersForJob = exports.calculateMatchScore = void 0;
...
const calculateMatchScore = (userProfile, jobEligibility) => {
    ...
    if (jobQuals.length === 0) {
        ...
        const jobNeedsCS = jobQuals.some((q) => CS_QUALIFICATIONS.has(q));
    ...
    if (jobStreams.length === 0) {
        ...
        const jobNeedsCSStream = jobStreams.some((s) => CS_STREAMS.has(s));
        const userIsEng = ENGINEERING_STREAMS.has(userStream);
        const jobNeedsEng = jobStreams.some((s) => ENGINEERING_STREAMS.has(s));
    ...
    if (jobYears.length === 0) {
        ...
        const closestYear = jobYears.reduce((prev, curr) => Math.abs(curr - userYear) < Math.abs(prev - userYear) ? curr : prev);
    ...
    if (userSkills && userSkills.length > 0) {
        ...
        const skillWords = userSkills.split(/[,\s]+/).filter((s) => s.length > 2);
        const matchedSkills = [];
 ...
 */
const findMatchingUsersForJob = async (jobId, limit = 100, minScore = 50) => {
    try {
        ...
        const userProfiles = await UserProfile_1.UserProfile.find(dbQuery)
        ...
        for (const profile of userProfiles) {
            const populatedUser = profile.userId;
            if (!populatedUser || !populatedUser.name || !populatedUser.email)
                continue;
            // Only alert users with active subscriptions
            if (populatedUser.subscriptionStatus !== 'active')
                continue;
            const { score, reasons } = (0, exports.calculateMatchScore)(profile, jobEligibility);
            if (score >= minScore) {
                matches.push({
        ...
        logger_1.logger.error('findMatchingUsersForJob failed', {
 ...
 */
const findMatchingJobsForUser = async (userId, limit = 20, minScore = 50) => {
    try {
        ...
        for (const job of jobs) {
            ...
            const { score, reasons } = (0, exports.calculateMatchScore)(profile, eligibility);
            if (score >= minScore) {
                matches.push({
        ...
        logger_1.logger.error('findMatchingJobsForUser failed', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId
        });
        throw error;
...
const getJobRecommendations = async (userId, options = {}) => {
    ...
    try {
        ...
        logger_1.logger.error('getJobRecommendations failed', { error, userId });
        return [];
    }
};
